...
export function FormatValidatorDialog({ onClose }: FormatValidatorDialogProps) {
  ...
  return createPortal(
    <div className="no-drag fixed inset-0 z-50 flex items-center justify-center bg-slate-950/18 px-5">
      <section
        className="flex h-[580px] w-full max-w-[720px] flex-col overflow-hidden rounded-lg border border-slate-200 bg-white shadow-[0_24px_72px_rgba(15,23,42,0.22)]"
        role="dialog"
        aria-modal="true"
      >
        ...
        <div className="flex shrink-0 items-center gap-2 border-b border-slate-100 px-5 py-3">
          <button
            type="button"
            className="inline-flex h-8 items-center gap-2 rounded-md border border-slate-200 bg-white px-3 text-[12px] font-medium text-slate-700 transition hover:bg-slate-50"
          ...
          {folderPath ? (
            <div className="min-w-0 flex-1 truncate text-[12px] text-slate-500">{folderPath}</div>
          ) : (
                ...
                <table className="w-full text-[12px]">
                  <thead>
                    <tr className="border-b border-slate-200 bg-slate-50">
                      ...
                      <th className="w-[90px] px-3 py-2 text-left font-medium text-slate-600">
                        {t("tools.formatValidator.colExtension")}
                      </th>
                      <th className="w-[90px] px-3 py-2 text-left font-medium text-slate-600">
                        {t("tools.formatValidator.colActualFormat")}
                      </th>
                      <th className="w-[100px] px-3 py-2 text-left font-medium text-slate-600">
                  ...
                  <tbody>
                    {mismatches.map((item) => (
                      ...
                      >
                        <td className="max-w-0 truncate px-3 py-2 text-slate-800">
                          {item.filePath}
                        </td>
                        <td className="px-3 py-2 text-slate-600">.{item.currentExtension}</td>
                        <td className="px-3 py-2 text-slate-700">{item.actualFormat}</td>
        ...
        <div className="flex shrink-0 items-center justify-between border-t border-slate-200 px-5 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              className="inline-flex h-9 items-center gap-2 rounded-md border border-slate-900 bg-slate-900 px-3 text-[13px] font-medium text-white transition hover:bg-slate-800 disabled:opacity-50"
