...
const TEXTGEN_BASE_URL: &str = "http://127.0.0.1:5005";
...
async fn discover_model(client: &reqwest::Client) -> Option<String> {
    let response = client
        .get(format!("{TEXTGEN_BASE_URL}/v1/models"))
        .send()
        .await
        .ok()?;
...
async fn generate_chat(
    ...
    let request = ChatRequest {
        model: discover_model(client).await,
        messages: vec![ChatMessage {
            role: "user".to_owned(),
            content,
        }],
    ...
    let response = client
        .post(format!("{TEXTGEN_BASE_URL}/v1/chat/completions"))
        .json(&request)
        .send()
        .await
        .map_err(|error| AppError::InvalidInput(format!("Textgen request failed: {error}")))?;
    ...
    if !status.is_success() {
        ...
        return Err(AppError::InvalidInput(if detail.is_empty() {
            format!("Textgen request failed with status {status}")
        } else {
            format!("Textgen request failed with status {status}: {detail}")
        }));
    }

    let payload: ChatResponse = response.json().await.map_err(|error| {
        AppError::InvalidInput(format!("Textgen response parse failed: {error}"))
    })?;
    let text = extract_content(payload);
    if text.is_empty() {
        return Err(AppError::InvalidInput(
            "Textgen response did not contain text".to_owned(),
        ));
    }
