...
const SETTINGS_FILE: &str = "deepseek-settings.json";
const DEFAULT_BASE_URL: &str = "https://api.deepseek.com";
const DEFAULT_MODEL: &str = "deepseek-v4-flash";
const DEFAULT_AVAILABLE_MODELS: [&str; 2] = ["deepseek-v4-flash", "deepseek-v4-pro"];
...
pub fn default_settings() -> DeepSeekSettings {
    DeepSeekSettings {
        api_key: String::new(),
        base_url: String::new(),
...
pub fn load_settings(dirs: &AppDirs) -> AppResult<DeepSeekSettings> {
    let path = dirs.config.join(SETTINGS_FILE);
    if !path.exists() {
        return Ok(default_settings());
    }

    let mut settings: DeepSeekSettings = serde_json::from_str(&fs::read_to_string(path)?)?;
...
fn normalize_settings(settings: &mut DeepSeekSettings) {
